...
$alpha = [a-zA-Z]
  ...
  @reservedid = as|case|class|data|default|deriving|do|else|hiding|if|
...
data Lexeme = L AlexPosn LexemeClass String deriving Show
...
scanner str = runAlex str $ loop 0
  where
  loop i = do
    tok@(L _ cl _) <- alexMonadScan; 
    if cl == LEOF
    then return (i,[])
    else do (i',ts) <- loop $! (i+1)
            return (i',tok:ts)
